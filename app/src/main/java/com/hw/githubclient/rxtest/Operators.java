package com.hw.githubclient.rxtest;
...
public class Operators {
    ...
    private static final class Consumer {
        ...
        public void execFilter() {
            producer.just().filter(new Predicate<String>() {
                @Override
                public boolean test(String s) throws Throwable {
                    return Integer.valueOf(s) > 1;
                }
            }).subscribe(
                    new io.reactivex.rxjava3.functions.Consumer<String>() {
                        @Override
                        public void accept(String s25) throws Throwable {
                            Log.i(TAG, "onNext " + s25);
                        }
                    },
                    (ex) -> Log.i(TAG, "onError " + ex.getMessage()),
                    () -> Log.i(TAG, "onComplete "));
        }
